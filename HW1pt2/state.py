...
def hdistance2(s): #This will be the Manhattan distance heuristic
    ...
    for i in range(len(s[0])):
        number =  s[0][i]
        currentRow = int(i/math.sqrt(len(s[0])))
        rowSupposedToBeOn = int(number/math.sqrt(len(s[0])))
        notinplace += abs(rowSupposedToBeOn - currentRow)
        currentColumn= int(i%math.sqrt(len(s[0])))
        columnSuppsoedtoBe = int(number%math.sqrt(len(s[0])))
        notinplace += abs(columnSuppsoedtoBe - currentColumn)
    return notinplace